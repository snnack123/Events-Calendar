    ...
    <div>
      ...
      {labelsEvents.map((label, idx) => (
        <label key={idx} className='items-center mt-3 block'>
          <input id={'label_'+idx} type="checkbox" className={`form-checkbox h-5 w-5 text-${label}-400 rounded focus:ring-0 cursor-pointer`} onClick={(e) => updateLabel(label, e.target.id)}
          />
